...
function TopBackground() {
    return (
        <div className='pointer-events-none absolute inset-0 w-full h-full z-0 flex items-center justify-center'>
            ...
            <div className='flex items-center justify-center rounded-full w-11 h-11 md:w-14 md:h-14  bg-opacity-10 absolute top-4 left-8 sm:top-10 sm:left-[780px]'>
                <Image src={"/icons/rocket_icon.svg"} alt='rocket_icon'
                    width={28} height={28}
                    className='w-[28px] h-[28px] sm:w-[46px] sm:h-[46px] sm:drop-shadow-lg sm:opacity-100 opacity-80 animate-bounce' />
            </div>
            ...
            <div className='flex items-center justify-center rounded-full w-11 h-11 md:w-14 md:h-14  bg-opacity-10 absolute bottom-8 right-8 sm:bottom-40 sm:right-[180px] z-50'>
                <Image src={"/icons/bracket_icon.svg"} alt='bracket_icon'
                    width={28} height={28}
                    className='w-[28px] h-[28px] sm:w-[46px] sm:h-[46px] sm:drop-shadow-lg sm:opacity-100 opacity-80 animate-caret-blink' />
            </div>
            {/* Github icon - bottom left, responsive and popping on desktop */}
            <div className='flex items-center justify-center rounded-full w-11 h-11 md:w-14 md:h-14  bg-opacity-10 absolute bottom-24 left-4 sm:bottom-[150px] sm:left-[120px]'>
                <Image src={"/icons/github_icon.svg"} alt='github_icon'
                    width={28} height={28}
                    className='w-[28px] h-[28px] sm:w-[46px] sm:h-[46px] sm:drop-shadow-lg sm:opacity-100 opacity-80 animate-bounce' />
            </div>
            {/* Electricity icon - top right, responsive and popping on desktop */}
            <div className='flex items-center justify-center rounded-full w-11 h-11 md:w-14 md:h-14  bg-opacity-40 absolute top-24 right-4 sm:top-[150px] sm:right-[250px] opacity-90 sm:opacity-100 '>
                <Image src={"/icons/electricity_icon.svg"} alt='electricity_icon'
                    width={28} height={28}
                    className='w-[28px] h-[28px] sm:w-[46px] sm:h-[46px] sm:drop-shadow-lg animate-ping' />
            </div>
